...
| 2 | `/api "Add Tender with #[ApiFilter(SearchFilter::class)] (legacy)"` | The legacy pattern is caught — either by the implementer self-audit (Step 5, via `anti-patterns-audit`) or by the gatekeeper full pass. Implementer rewrites to `parameters: [new QueryParameter(...)]`. |

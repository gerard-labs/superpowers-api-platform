...
## Bounded contexts variant (Hexagonal evolved)

An alternative split — **one context per business domain**, each autonomous:

```
src/
├── Shared/                     # cross-context VOs, exceptions, utils
├── ProductContext/
...
│   ├── Repository/
│   ├── Service/
│   └── Contract/               # ⚠️ public surface
├── OrderContext/                # same shape
├── CustomerContext/
└── ErpContext/                  # ERP adapter, isolated
```

**Golden rule**: a context depends only on the `Contract/` of other contexts. Never directly on their `Entity/` or `Service/` internals.

### Splitting criteria

- Functional cohesion.
- Team autonomy (different teams → different contexts).
- Tech vs business split (`ErpContext` isolated from business core).
- Rate of change (stable code separated from volatile code).

### Enforcing boundaries — Deptrac

```yaml
# deptrac.yaml
parameters:
    layers:
        - { name: ProductContext,  collectors: [{ type: directory, regex: src/ProductContext/.* }] }
        - { name: OrderContext,    collectors: [{ type: directory, regex: src/OrderContext/.* }] }
        - { name: SharedContract,  collectors: [{ type: directory, regex: src/.+/Contract/.* }] }
    ruleset:
        OrderContext:   [SharedContract]
        ProductContext: [SharedContract]
```

CI fails if `OrderContext` imports an internal class from another context.

### ADR — translate to PHPStan rules

Architectural decisions (ADRs) only stick if **automatically enforced**. A custom PHPStan rule can forbid `EntityManager` injection into a Provider, force command DTOs to be `final readonly`, prevent a Doctrine entity from being a `#[ApiResource]`. CI fails on violation — the ADR becomes self-enforcing.

## Related skills
- `gerard:cqrs-and-handlers` — Command/Query separation works hand-in-hand with bounded contexts.
- `gerard:interfaces-and-autowiring` — the wiring layer (Symfony DI) is the adapter that plugs ports.
- `gerard:value-objects-and-dtos` — VOs live in `Shared/` or in the domain layer of each context.
- `gerard:quality-checks` — Deptrac + PHPStan custom rules belong here.
- `gerard:api-platform-resources` — API Platform resources are part of the **adapter** layer, not the domain.

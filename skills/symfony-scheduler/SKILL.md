...
allowed-tools:
  ...
  - Glob
  - Grep
description: Schedule recurring tasks with Symfony Scheduler — `#[AsSchedule]` provider, `RecurringMessage::every('1 hour')` / `cron('0 0 * * *')` / `trigger(...)` triggers, integration with Messenger transports, `messenger:consume scheduler_default` for the worker, custom scheduler IDs for parallel cron tracks, `between('09:00', '18:00')` business-hour windows. Trigger on "nightly cleanup", "every N minutes report", "cron job in Symfony", or "replace crontab with native code".
---

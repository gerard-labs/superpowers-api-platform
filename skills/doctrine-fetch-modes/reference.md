...
## API Platform 4.3 trap — `eager_loading.force_eager: true`

In API Platform 4.x, `eager_loading.force_eager: true` is the default. API Platform then eagerly loads **every** association in a single JOIN on each GET. On entities with many relations, `max_joins: 30` is hit quickly and an exception fires. Often counter-productive: hydrates fields that will not be serialized.
...
With `force_eager: false`, API Platform stays lazy by default; developers add targeted `addSelect()` join fetches in repositories or query extensions. Enable eager per property when really needed:
...
## Related skills
- `gerard:api-platform-performance` — full performance posture (force_eager, FrankenPHP worker, cache tags).
- `gerard:api-platform-pagination` — cursor pagination relies on indexed sort keys.
- `gerard:doctrine-relations` — fetch-mode choices live with the relationship declaration.

...
## BackedEnum — first-class citizen in API Platform 4.3

PHP 8.1+ `BackedEnum` is supported natively across the API Platform stack:

- **Serialization**: emitted as its scalar value (`string` or `int`).
- **OpenAPI**: documented as `enum: [draft, processed, …]`.
- **Doctrine persistence**: `#[ORM\Column(enumType: OrderStatus::class)]`.
- **Filtering**: native `BackedEnumFilter` in the modern `parameters: [QueryParameter]` pattern (cf. `gerard:api-platform-filters`).
...
**Always prefer `BackedEnum` over a free `string`** for any status / type / category exposed by the API.

## API Platform 4.3 — IRI-only relations in DTOs

DTOs accepted by API Platform operations resolve relations through IRIs, not scalar identifiers. Type relation fields as the target Resource, never as `int $customerId` / `string $productSku`:

```php
final readonly class CreateOrderInput
{
    public function __construct(
        #[Assert\NotNull] public Customer $customer,
        #[Assert\Count(min: 1)] #[Assert\Valid] public array $items,
        public ?Coupon $coupon = null,
    ) {}
}
```

Full pattern: `gerard:api-platform-dto-resources`.

### Object Mapper 4.3 — mechanical DTO ↔ Entity mapping

For simple field-rename / format mappings, the Symfony Object Mapper integration (`#[Map]`) avoids the Provider/Processor boilerplate entirely. Cf. `gerard:api-platform-dto-resources` §6.

## Related skills
- `gerard:api-platform-dto-resources` — Input/Output DTOs + Object Mapper 4.3.
- `gerard:api-platform-serialization` — `BackedEnum` in groups, `#[Context]`, MaxDepth.
- `gerard:api-platform-resources` — IRI-only relation rule.
- `gerard:api-platform-identifiers` — UUID v7 / ULID as VO-like identifiers.
- `gerard:doctrine-relations` — relationship modeling that the VO/DTO layer wraps.

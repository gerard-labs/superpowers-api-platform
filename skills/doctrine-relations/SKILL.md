...
allowed-tools:
  ...
  - Glob
  - Grep
description: Design Doctrine entity relationships — `OneToMany` / `ManyToOne` / `ManyToMany` / `OneToOne`, owning vs inverse side, `cascade` operations (`persist` / `remove` with justification), `orphanRemoval`, indexes (`#[ORM\Index]`), unique constraints, fetch mode per relation (LAZY default, EXTRA_LAZY for collections). Trigger when designing entities, adding a join, or auditing relations for N+1 risk.
---

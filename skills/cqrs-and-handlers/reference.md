...
### Bridging a Processor to a Command Bus
...
final readonly class CreateOrderProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $op, array $uriVars = [], array $ctx = []): object
    {
        $envelope = $this->commandBus->dispatch(new CreateOrder(
            customer: $data->customer,   // already resolved from IRI
            items:    $data->items,
        ));
...
The `doctrine_transaction` middleware on the bus guarantees atomicity.

### Native Messenger mode (no Processor needed for fire-and-forget)

For purely async operations, API Platform exposes a native Messenger mode through the operation attribute — no custom Processor required:
...
### Domain Events vs Messenger commands

| Mechanism | Synchronicity | Use case |
|---|---|---|
| Domain Event (Symfony EventDispatcher) | Synchronous, **same transaction** as the emitter | Atomic side effect (e.g. `OrderCreated` → email + ERP sync within the same transaction) |
| Messenger command/message | Async, with retry + DLQ | Effect can be deferred (export, reindex, third-party call) |

**Rule of thumb**: if the subscriber must be allowed to fail without breaking the main operation, it's Messenger. Otherwise, Event Dispatcher.

## Related skills
- `gerard:api-platform-state-processors` — Processor-as-bus-dispatcher pattern + native Messenger modes.
- `gerard:api-platform-resilience` — async writes for slow upstreams (HTTP 202 + Messenger).
...
- `gerard:ports-and-adapters` — bounded contexts and the public `Contract/` surface where commands live.

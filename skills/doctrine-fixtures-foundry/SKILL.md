...
allowed-tools:
  ...
  - Glob
  - Grep
description: Build deterministic test data with `zenstruck/foundry` factories — `Factory::createOne()`, `createMany()`, states, sequences, lifecycle callbacks, lazy / eager flush, relation factories. Use with `use Factories;` + `use ResetDatabase;` (or DAMA Doctrine Test Bundle transaction rollback). Trigger when bootstrapping a test suite, when manual fixture arrays explode, or when a test needs N entities with a specific shape.
---

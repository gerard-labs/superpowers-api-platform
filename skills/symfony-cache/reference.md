...
### Operation-level HTTP cache + cache tags
...
The response carries `Cache-Tags: products`. Varnish / Nginx purges every response tagged accordingly on a `PURGE /api/products/123` request.
...
### APCu cache for resource metadata

API Platform recomputes the resource metadata on each boot if no cache is configured. In production, always wire APCu (or another PSR-6 adapter):
...
### Dynamic `Last-Modified` listener

For collections where `Last-Modified` is `max(updatedAt)`, wire a `ViewEvent` listener that calls `Response::setLastModified()` and returns 304 if the request is fresh. The content-based **ETag is automatic** in API Platform — no listener needed for it.

## Related skills
- `gerard:api-platform-performance` — full performance posture (force_eager trap, FrankenPHP worker, cache tags + Varnish PURGE).
- `gerard:api-platform-resources` — `cacheHeaders` and `cacheTags` on operations.
- `gerard:doctrine-fetch-modes` — query-level cache hints (`HINT_READ_ONLY`).

...
## API Platform 4.3 integration

A clean way to apply rate limiting uniformly across every API resource is to **decorate the main State Provider** rather than wiring a kernel subscriber — the `parameters: [QueryParameter]` and `security:` operations both run through the provider chain, so a single decorator catches every code path without touching kernel events.
...
#[AsDecorator('api_platform.state_provider.main')]
final class AuthAwareRateLimitedProvider implements ProviderInterface
{
    public function __construct(
        #[AutowireDecorated] private ProviderInterface $decorated,
        private RateLimiterFactory $anonLimiter,
        private RateLimiterFactory $authLimiter,
        private Security $security,
    ) {}

    public function provide(\ApiPlatform\Metadata\Operation $op, array $uriVars = [], array $ctx = []): object|array|null
    {
        $user    = $this->security->getUser();
        $limiter = $user
            ? $this->authLimiter->create($user->getUserIdentifier())
            : $this->anonLimiter->create($ctx['request']->getClientIp());

        if (!$limiter->consume(1)->isAccepted()) {
            throw new TooManyRequestsHttpException(null, 'Rate limit exceeded.');
        }
        return $this->decorated->provide($op, $uriVars, $ctx);
    }
}
```

Dedicated limiter for write operations:

```yaml
framework:
    rate_limiter:
        api:        { policy: sliding_window, limit: 1000, interval: '1 hour' }
        api_write:  { policy: sliding_window, limit: 100,  interval: '1 hour' }
```

## Related skills
- `gerard:api-platform-security` — login throttling, JWT, CORS.
- `gerard:api-platform-state-providers` — Provider decoration pattern (here applied to `api_platform.state_provider.main`).
- `gerard:api-platform-resilience` — Circuit Breaker + async writes pair with rate limiting for resilient APIs.
- `gerard:api-platform-mcp` — dedicated `/mcp` rate limit for AI-agent calls.

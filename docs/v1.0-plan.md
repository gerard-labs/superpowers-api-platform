...
│   ├── writing-plans/
│   ├── executing-plans/
│   ├── anti-patterns-audit/                 # NEW : ex /self-audit-api standalone (cross-cutting, plain top-level — Claude Code loader doesn't recurse)
│   └── goal-patterns/                       # NEW : templates de condition /goal (cross-cutting, plain top-level)
...
| `/self-audit-api` | ❌ supprimé. Devient skill standalone `anti-patterns-audit/`. |
...
**Ajoutés** :
- `anti-patterns-audit/` — skill standalone pour audit hors-pipeline (ex /self-audit-api)
- `goal-patterns/` — templates de condition `/goal` (5 patterns : feature, refactor, migration, hardening, docs-only)
...
□ Création structure cible :
  ...
  □ mkdir skills/anti-patterns-audit
  □ mkdir skills/goal-patterns
...
□ commands/api.md :
  ...
  □ Body :
    ...
    □ Compose /goal condition depuis skills/goal-patterns/ (template selon story shape)
...
□ Crée skills/anti-patterns-audit/SKILL.md :
...
□ Crée skills/goal-patterns/SKILL.md :
